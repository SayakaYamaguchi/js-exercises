...
class PageState {
  ...
  // このファクトリメソッドはURL から状態を取得する。URL から有効な状態が
  ...
  static fromURL(url) {
    ...
    // 有効な値を取得できた場合はPageSate オブジェクトを返す。そうでない場合はnull。
    return isNaN(s.cx) ||
      isNaN(s.cy) ||
      isNaN(s.perPixel) ||
...
class MandelbrotCanvas {
  constructor(canvas) {
    ...
    // URL から状態を初期化するか、初期状態で開始する。
    this.state = PageState.fromURL(window.location) || PageState.initialState();
    // この状態を履歴機構を使って保存する。
    history.replaceState(this.state, "", this.state.toURL());
    // canvas の大きさを設定して、それを覆うtile の配列を取得する。
    this.setSize();
    // そして、マンデルブロ集合をcanvas に描画する。
    this.render();
  }

  // canvas の大きさを設定して、Tile オブジェクトの配列を初期化する。この
  ...
  // この関数は、PageState に変更を行い、その新しい状態を使って
  // マンデルブロ集合を再描画し、さらにその新しい状態を
  // history.pushState() で保存する。1 番目の引数が関数の場合、この関数は
  // 状態オブジェクトを引数にして呼び出される。この関数が、その状態に変更を
  // 加える。第1 引数がオブジェクトの場合は、そのオブジェクトのプロパティを
  // 状態オブジェクトにコピーするだけ。省略可能な第2 引数がfalse の場合、
  // 新しい状態は保存されない（popstate イベントに応答してsetState を
  // 呼び出すときにはこのようにする）。
  setState(f, save = true) {
    ...
    this.render();
    // 通常は新しい状態を保存する。ただし、第2 引数にfalse が指定されて
    // 呼び出された場合は保存しない（例えば、popstate イベントを処理するとき）。
    if (save) {
      history.pushState(this.state, "", this.state.toURL());
    }
  }
  // このメソッドは、PageState オブジェクトで指定されたマンデルブロ集合の
  // 一部をcanvas に描画する。このメソッドは、コンストラクタや、状態が
  // 変化したときのsetState() や、canvas の大きさが変化したときのresize
  // イベントハンドラから呼び出される。
  render() {
    ...
    // ROWS*COLS のTile それぞれに対して、mandelbrotWorker.js のコードに
    ...
    // する。各レスポンスは、各Tile に対する計算結果。mandelbrotWorker.js の
    // コードを思い出せば、各レスポンスには、Tile オブジェクト、ピクセル値の代わりに
    // 反復回数を含むImagaData オブジェクト、また、そのTile の最小および
    // 最大反復回数が含まれている。
    this.pendingRender = Promise.all(promises)
      .then((responses) => {
        // まず、すべてのTile に対する全体の反復回数の最大値と最小値を
        ...
        // ここで、ワーカーの生の反復回数を、canvas に表示されるピクセルの色に
        // 変換する方法が必要になる。すべてのピクセルの反復回数はmin とmax の
        ...
        // 最後に、putImageData() を使って、すべてのimageData オブジェクトを、
        // canvas の対応するtile に描画する（ただし、最初に、pointerdown
        // イベントハンドラによって設定されている可能性のあるcanvas 上の
  ...
  // canvas のサイズを変更して、マンデルブロ集合を再描画するのは、1 秒間に
  // 何度も実行できるような処理ではないので、タイマーを使用して、最後に
  // サイズ変更イベントを受信してから200ms 経過するまで、サイズ変更の処理を
  // 延期する。
  handleResize(event) {
    ...
    this.resizeTimer = setTimeout(() => {
      ...
      this.setSize(); // canvas とtile のサイズを変更する。
      this.render(); // 新しいサイズで再描画する。
    }, 200);
  }
  // ユーザがキーを押下した場合、このイベントハンドラが呼び出される。
  // 押下されたキーに応じてsetState() を呼び出し、setState() は
  // 新しい状態を描画し、URL を更新し、ブラウザ履歴に状態を保存する。
  handleKey(event) {
    switch (event.key) {
      case "Escape": // ESC キーを押下した場合は、初期状態に戻る。
  ...
  // このメソッドは、canvas 上でpointerdown イベントが発生したときに
  // 呼び出される。pointerdown イベントは、ズームのジェスチャー
  // （クリックやタップ）や、パンのジェスチャー（ドラッグ）の開始時に
  // 発生する。このハンドラは、ジェスチャーの残りの部分に反応するために、
  // pointermove とpointerup イベントハンドラを登録する（これら2 つの
  // ハンドラは、pointerup でジェスチャーが終了すると削除される）。
  handlePointer(event) {
    // 最初にポインタがダウンしたときのピクセル座標と時間。
    // canvas はウィンドウと同じ大きさなので、このイベント座標は
    // canvas 座標でもある。
    ...
    const pointerMoveHandler = (event) => {
      ...
      // 通常のクリックではないない場合は、CSS を使って表示をパンする
      // （pointerup イベントが発生したときに、実際に描画する）。
    ...
    // ユーザがジェスチャーを開始すると、それに続くpointermove と
    // pointerup イベントのハンドラを登録する。
