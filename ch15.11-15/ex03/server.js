...
function corsMiddleware(_url, _req, res) {
  ...
  res.setHeader("Access-Control-Allow-Origin", "http://localhost:3000");              // リクエストの許可
  res.setHeader("Access-Control-Allow-Methods", "GET, POST, PATCH, DELETE, OPTIONS"); // 許可するHTTPメソッド
  res.setHeader("Access-Control-Allow-Headers", "Content-Type");                      // クライアントから送信されるリクエストヘッダー（Content-Type）を許可
  res.setHeader("Access-Control-Allow-Credentials", "true");                          // クッキーを含むリクエストを許可
  return true;
}
  ...
  http
    .createServer(async function (req, res) {
      await routes(
        ...
        ["OPTIONS", "/api/*", nopHandler, cors], // CORS プリフライト対応
        ...
        ["GET", "/*", serveContentsHandler] // 他の静的ファイルのハンドリング
      )(req, res);
    })
    .listen(3001);
