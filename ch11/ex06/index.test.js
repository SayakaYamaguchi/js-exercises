...
describe("isEmailAddress", () => {
    it("checks if given string is e-mail address or not", () => {
        ...
        expect(isEmailAddress("foo..bar@example.com")).toBe(false); // .が連続はfalse
        expect(isEmailAddress("foo.@example.com")).toBe(false); //　.が@の直前はfalse
        expect(isEmailAddress(".foo@example.com")).toBe(false); //　.が先頭はfalse
        expect(isEmailAddress("0123456789012345678901234567890123456789012345678901234567890123@example.com")).toBe(true);  
        expect(isEmailAddress("01234567890123456789012345678901234567890123456789012345678901234@example.com")).toBe(false);    // @以前が65文字以上はfalse
        ...
        expect(isEmailAddress("a@0123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012")).toBe(false);  // @以降253文字以上はfalse
