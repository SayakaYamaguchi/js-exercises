...
export function dateObj(){
    ...
    const matchMonth = currentDate.match(/-(\d{2})-/);    // -で囲まれた数字を抽出する
    const matchYear = currentDate.substring(0, 4);
    const month = matchMonth;
    ...
    const lastMonthStartDate = new Date(matchYear, month, 1);
    console.log(lastMonthStartDate);
    return(lastMonthStartDate);
//    const month = parseInt(match[1], 10); // 抽出した数字を整数に変換
//    if(match && match[1]){
