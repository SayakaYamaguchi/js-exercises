/**
 * AbstractSetクラスではhas()中小メソッドのみを定義する
 */
class AbstractSet {
 ...
 * このセットはある他のセットのメンバーではない値すべてがメンバーとなる
 * このセットは他のセットの状態によって定義されるセットなので書き込むことはできない
 * また、メンバ0は無限に存在するので列挙もできない
 * このセットを使ってできることはメンバーに含まれるかどうかを調べることと
...
class NotSet extends AbstractSet {
  ...
  // またObjectのtoString()メソッドもオーバーライドする
  toString() {
 ...
 * また、この 2 つの抽象メソッドを使って、isEmpty()、toString()、
 ...
 * シングルトンセットは、メンバーが 1 つしかない読み出し専用のセット。
 */
class SingletonSet extends AbstractEnumerableSet {
  ...
  // このサブクラスでは以下の 3 つのメソッドを実装する。この 3 つのメソッドを使って
 ...
 * また、この 2 つのメソッドを使って、add()、subtract()、intersect()
